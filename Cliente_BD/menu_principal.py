...
def usuarios_insertar(nombre: str, apellido: str, num_contacto: str, roles_id_roles: int, created_by: str = "script") -> bool:
    """
    Llama a sp_usuarios_insertar para crear un nuevo usuario.
    """
    args = [nombre, apellido, num_contacto, roles_id_roles, created_by]
...
def materiales_insertar(nombre: str, composicion: str, precio_unidad: float, created_by: str = "script") -> bool:
    args = [nombre, composicion, precio_unidad, created_by]
...
def objetos_insertar(nombre: str, descripcion: str, id_material: int, id_basurero: int, created_by: str = "script") -> bool:
    args = [nombre, descripcion, id_material, id_basurero, created_by]
    return _call_proc_no_results("sp_objetos_insertar", args)


def objetos_borrado_logico(id_objeto: int, updated_by: str = "script") -> bool:
    # Nota: en el SP original la columna se llama 'id_objetos' en la cláusula WHERE.
    # Aquí asumimos que el SP acepta un INT y funciona correctamente.
...
def menu_tabla_usuarios():
    while True:
        ...
        if op == "1":
            ...
            ok = usuarios_insertar(nombre, apellido, num_contacto, role_id, created_by)
            print("OK" if ok else "Error")
        elif op == "2":
    ...
    while True:
        ...
        if op == "1":
            ...
            try:
                ...
                continue
            created_by = input("Creado por: ").strip() or "script"
            ok = materiales_insertar(nombre, composicion, precio, created_by)
            print("OK" if ok else "Error")
        elif op == "2":
    ...
    while True:
        ...
        if op == "1":
            ...
            try:
                ...
                continue
            created_by = input("Creado por: ").strip() or "script"
            ok = objetos_insertar(nombre, descripcion, id_mat, id_bas, created_by)
            print("OK" if ok else "Error")
        elif op == "2":
